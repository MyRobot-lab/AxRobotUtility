...
#
points=dict()                          #**儲存所有單點
_joint=["joint",0.,0.,0.,0.,0.,0.,0.]  #**目前的關節座標(角度回授)
_track=["joint",0.,0.,0.,0.,0.,0.,0.]  #**目前的關節座標(角度命令)
...
def pTo(p,z):                         #***** 轉換單點 *****
    pSet("###CONV",p); return(pGet("###CONV",z))
def pGet(s,z=""):                     #***** 取得單點 *****
...
#
dots=list()
def pDot(p=None,z="base"):
    if isinstance(p,int):    #**讀取dots[n]
        n=p;
        if n<0:              #**清除dots[]
            dots.clear(); return None
        if n<len(dots):   return(dots[n])
        else:             return(_none)
